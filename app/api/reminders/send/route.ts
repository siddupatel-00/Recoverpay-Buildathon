 ...
 * Guardrails: PAID → 400 blocked; attempts >= 3 → ESCALATED; else AI draft + store.
 */
export async function POST(req: NextRequest) {
  ...
  if (inv.status === "PAID") {
    await audit(inv.id, "REMINDER_BLOCKED", "Stopping rule: invoice already PAID, reminder blocked");
  ...
  if (inv.attempts >= MAX_ATTEMPTS) {
    ...
    await audit(inv.id, "ESCALATED", `Max ${MAX_ATTEMPTS} retries exhausted — routed to human`);
    return NextResponse.json({ error: "Max retries exhausted — escalated to human", escalated: true }, { status: 400 });
  }
  ...
  await audit(inv.id, "REMINDER_SENT", `Attempt #${attempt} via ${source}: ${text.slice(0, 140)}`);
  return NextResponse.json({ ok: true, attempt, message: text, source, status: nextStatus });
}
